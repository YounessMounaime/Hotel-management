...
const MesReservations = () => {
  ...
  const getReservationData = () => {
    axios.get('https://localhost:7172/api/Reservation')
      .then((result) => {
        setData(result.data);
      })
      .catch((error) => {
  ...
  return (
    <div>
      <section className="breadcrumb_area">
        ...
        <h1 style={{textAlign: 'center'}}>Liste des Chambres Réservées</h1>
  ...
  {data && data.length > 0 ? (
    ...
    <tr>
      <td colSpan="8">Aucune réservation disponible.</td>
        ...
        <hr/>
        <br/>
        <h1 style={{textAlign: 'center'}}>Liste des Salles Réservées</h1>
        <table className="content-table" style={{marginRight: 'auto', marginLeft: 'auto'}}>
            <thead>
                <tr>
                    <th>Date d'Arrivée</th>
