 ...
 const navigate = useNavigate();
  const handleLogin = async () => {
    try {
      ...
      if (email === 'admin@email.com' && password === 'Promo_Emsi_2023_2024') {
        console.log('Login successful.');
        navigate('/AccueilAdminSup');
        return;
      }
      const response = await fetch(`https://localhost:7172/api/Utilisateur/login?email=${encodeURIComponent(email)}&password=${encodeURIComponent(password)}`, {
        method: 'POST', 
        headers: {
